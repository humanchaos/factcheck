...
def load_source_config(config_path: Optional[str] = None) -> Tuple[set, set, set]:
    ...
    """
    # Search order: explicit path > quality-gate-sources.json > sources.json > defaults
    search_paths = []
    if config_path:
        search_paths.append(config_path)
    search_paths.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'quality-gate-sources.json'))
...
SPEAKER_PATTERNS = [
    ...
    r'kritisiert die',
    r'bezeichnet .* als',
    # Channel/platform self-promotion
    r'channel gained',
    r'channel offered',
    r'channel provides',
    r'channel.* reached',
    r'channel has become',
    r'kanal hat .* gewonnen',
    r'kanal bietet',
    r'audience .* billion',
    r'audience .* million',
    # Speaker biography (who the speaker is/was)
    r'^the speaker was',
    r'^the speaker is',
    r'^the host was',
    r'^the host is',
    # Declarations of intent / political promises
    r'will achieve',
    r'will be achieved',
    r'will unconditionally',
    r'will liberate',
    r'will prevail',
    r'goals .* will .* achieved',
    r'goals .* operation',
    r'parity remains',
    r'werden .* erreicht',
    r'werden .* befreit',
...
def run_structural_checks(claim: dict, ci: int, chi: int) -> List[Violation]:
    ...
    if conf == 0.5:
        violations.append(Violation(
            ...
            auto_fixable=True,
            fix_description='Recalculate confidence from source tiers'
        ))
    if conf == 0.28:
        violations.append(Violation(
            'S3_confidence_range', Severity.WARNING, ci, chi,
    ...
    if orig and not cleaned:
        violations.append(Violation(
            ...
            auto_fixable=True,
            fix_description='Copy originalClaim to cleanedClaim as baseline'
        ))
    ...
    if sources:
        ...
        if api_leak:
            violations.append(Violation(
                'S6_source_api_leak', Severity.WARNING, ci, chi,
                ...
                auto_fixable=False,
                fix_description='Fixed at runtime — new captures extract actual URLs from groundingMetadata'
            ))
    ...
    if len(exp) > 30:
        ...
        if transcript_lang == 'de' and exp_lang == 'en':
            violations.append(Violation(
                ...
                auto_fixable=True,
                fix_description='Tag explanation with language mismatch marker'
            ))
    ...
    for chi, ci, claim in all_claims:
        ...
        if verdict in ('true', 'false') and conf < 0.3:
            violations.append(Violation(
                ...
                auto_fixable=True,
                fix_description='Floor confidence to 0.4 for definitive verdicts'
            ))
        if verdict == 'unverifiable' and conf > 0.5:
            violations.append(Violation(
                ...
                auto_fixable=True,
                fix_description='Cap confidence to 0.4 for unverifiable verdicts'
            ))
    ...
    for chi, ci, claim in all_claims:
        ...
        if not check_explanation_verdict_alignment(exp, verdict):
            violations.append(Violation(
                ...
                auto_fixable=True,
                fix_description='Override verdict to match evidence direction in explanation'
            ))
...
def apply_fixes(data: List[dict], violations: List[Violation]) -> Tuple[List[dict], int]:
    ...
    # Fix C3: Clamp confidence to verdict-coherent bounds
    if 'C3_confidence_coherence' in by_type:
        for chunk in fixed:
            for claim in chunk.get('claims', []):
                v = claim.get('verification', {})
                verdict = v.get('verdict', '')
                ...
                if verdict in ('true', 'false') and conf < 0.3:
                    v['confidence'] = 0.4
                    fix_count += 1
                elif verdict == 'unverifiable' and conf > 0.5:
                    v['confidence'] = 0.4
                    fix_count += 1
    
    # Fix S4: Copy originalClaim to cleanedClaim if missing
    if 'S4_cleaned_claim_present' in by_type:
        for chunk in fixed:
            for claim in chunk.get('claims', []):
                orig = claim.get('originalClaim', claim.get('claim', ''))
                if orig and not claim.get('cleanedClaim', ''):
                    claim['cleanedClaim'] = orig
                    fix_count += 1
    
    # Fix S3: Recalculate hardcoded confidence (0.5 / 0.28) from source tiers
    if 'S3_confidence_range' in by_type:
        TIER_SCORES = {1: 0.75, 2: 0.3}
        for chunk in fixed:
            for claim in chunk.get('claims', []):
                v = claim.get('verification', {})
                conf = v.get('confidence', -1)
                if conf in (0.5, 0.28):
                    ...
                    if sources:
                        total = sum(
                            TIER_SCORES.get(
                                (s.get('tier', 4) if isinstance(s, dict) else 4),
                                0.1
                            )
                            for s in sources
                        )
                        new_conf = min(0.95, max(0.1, total))
                        v['confidence'] = round(new_conf, 2)
                    else:
                        v['confidence'] = 0.1
                    fix_count += 1
    
    # Fix M3: Tag language-mismatched explanations
    if 'M3_explanation_language' in by_type:
        for chunk in fixed:
            for claim in chunk.get('claims', []):
                v = claim.get('verification', {})
                exp = v.get('explanation', '')
                if exp and not exp.startswith('[DE→EN]'):
                    v['explanation'] = f'[DE→EN] {exp}'
                    fix_count += 1
    
    # Fix C5: Override verdict to match evidence direction in explanation
    if 'C5_explanation_verdict_alignment' in by_type:
        positive_signals = ['confirmed', 'supported', 'bestätigt', 'belegt', 'korrekt',
                           'is true', 'is correct', 'stimmt', 'trifft zu']
        negative_signals = ['contradicted', 'false', 'falsch', 'widerlegt', 'not supported',
                           'nicht bestätigt', 'incorrect', 'not true']
        for chunk in fixed:
            for claim in chunk.get('claims', []):
                v = claim.get('verification', {})
                verdict = v.get('verdict', '')
                exp = str(v.get('explanation', '') or '').lower()
                has_pos = any(s in exp for s in positive_signals)
                has_neg = any(s in exp for s in negative_signals)
                if verdict == 'false' and has_pos and not has_neg:
                    v['verdict'] = 'true'
                    v['confidence'] = min(v.get('confidence', 0.5), 0.5)
                    v['explanation'] = f'[C5 AUTO-FIX: verdict flipped to match evidence] {v.get("explanation", "")}'
                    fix_count += 1
                elif verdict == 'true' and has_neg and not has_pos:
                    v['verdict'] = 'false'
                    v['confidence'] = min(v.get('confidence', 0.5), 0.5)
                    v['explanation'] = f'[C5 AUTO-FIX: verdict flipped to match evidence] {v.get("explanation", "")}'
                    fix_count += 1
    
    # Fix E1: Remove speaker-leaked claims
    if 'E1_speaker_action_leak' in by_type:
        for chunk in fixed:
            to_remove = []
            for ci, claim in enumerate(chunk.get('claims', [])):
                orig = claim.get('originalClaim', claim.get('claim', '')).lower()
...
def run_quality_gate(input_file: str, fix: bool = False, output_file: str = None, strict: bool = False) -> RunReport:
    ...
    if fix:
        ...
        # Re-analyze corrected data to show improvement
        fixed_report = run_quality_gate(out_path, fix=False)
        delta = fixed_report.overall_score - report.overall_score
        arrow = '↑' if delta > 0 else '→' if delta == 0 else '↓'
        print(f"\n  Applied {fix_count} auto-fixes → {out_path}")
        print(f"  Score: {report.overall_score:.1f} [{report.grade}] {arrow} {fixed_report.overall_score:.1f} [{fixed_report.grade}] (Δ{delta:+.1f})")
